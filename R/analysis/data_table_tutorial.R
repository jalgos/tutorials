...
#To load the data and sources and see a few example visualizations, run the following command in a R shell opened at the root of the project (../R_tuto/) and press keys until you are back on a terminal prompt.
...
#If you want to access the columns of a datatable without knowing beforehand the column names (it happens a lot when doing advanced stuff) there are several ways that I will comment later.
...
#The order of arguments in [] is i,j,by,...(other arguments that I will explain later). To use a similar logic to dataframe's we omitted the names of the argument so far.
...
#######
#I walked you through a few examples of datatable usage. This tutorial is not thorough but it should cover most of the basic usage.
